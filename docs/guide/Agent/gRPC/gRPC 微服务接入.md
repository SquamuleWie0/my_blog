...
await
= 等异步操作完成，不等于 gRPC，但 gRPC 异步调用通常需要 await
```
...
gRPC + protobuf 中间层
request 对象 → bytes → 网络 → bytes → request 对象
...
gRPC + protobuf 中间层
response 对象 → bytes → 网络 → bytes → response 对象
...
```text
proto：设计图，定义服务、方法、请求、响应
pb2：根据 proto 生成的 message 类
pb2_grpc：根据 proto 生成的 stub、servicer、注册函数
stub：客户端拿来调用服务端的代理对象
channel：客户端到服务端的连接
protobuf：对象和 bytes 之间的编码格式
gRPC：远程调用通道和运行机制
servicer：服务端接住 RPC 请求的入口
service：项目里真正做业务的代码
```

这里容易混的是 `proto` 和运行时对象。

`.proto` 文件本身不是运行时对象，它只是设计图。

真正运行时用到的是生成后的 Python 类：

```text
ChatRequest          # pb2 里的请求对象
ChatResponse         # pb2 里的响应对象
AgentServiceStub     # pb2_grpc 里的客户端代理
AgentServicer        # 自己写的服务端实现
```
...
→ 生成 pb2 / pb2_grpc
→ 客户端用 stub 发请求
→ gRPC / protobuf 负责传输和编解码
→ 服务端 servicer 接请求
→ service 层执行业务
```

这也是 Agent 微服务接入 gRPC 时最重要的一条边界：
